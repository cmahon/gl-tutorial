...
initialise = runMaybeT $ do
  png ← liftIO $ readPng =<< getDataFileName "haskell.png"
  ...
  vs  ←   MaybeT $ loadAndCompileShader
            GL_VERTEX_SHADER =<< getDataFileName "vertexShader.glsl"
  fs  ←   MaybeT $ loadAndCompileShader
            GL_FRAGMENT_SHADER =<< getDataFileName "fragmentShader.glsl"
